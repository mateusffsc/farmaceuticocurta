...
export default function ClientAddMedicationModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const dosageWithUnit = `${formData.dosage.trim()}${formData.unit}`;
      ...
      const { error: medError } = await supabase
        ...
        .insert([{
          ...
          total_quantity: formData.total_quantity.trim() ? parseInt(formData.total_quantity, 10) : null,
          treatment_duration_days: parseInt(formData.treatment_duration_days),
          start_date: formData.start_date,
          notes: formData.notes.trim() || null,
          recurrence_type: formData.recurrence_type,
          recurrence_custom_dates: formData.recurrence_type === 'custom' ? customDates.filter(d => d).join(', ') : null,
        }]);
